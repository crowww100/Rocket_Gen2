...
import pickle #read cameraMatrix
...
#define posix MessageQueues
#mq_horizon = posix_ipc.MessageQueue("/queue_horizon", 0, 600, 1 , 256, True, True)
...
if __name__ == "__main__":
    ...
    #eki: load cameraMatrix for fisheye correction
    # Kameramatrix laden
    with open('cameraMatrix.pkl', 'rb') as f:
       cameraMatrix = pickle.load(f)

    # Verzerrungskoeffizienten laden
    ...
    while 1:
       ...
       h,  w = frame.shape[:2]
       # Erstellen der Korrekturmatrix und ROI
       newCameraMatrix, roi = cv2.getOptimalNewCameraMatrix(cameraMatrix, dist, (w,h), 1, (w,h))
       ...
       # Anwenden der Korrekturmatrix auf das Bild
       dst = cv2.remap(frame, mapx, mapy, cv2.INTER_LINEAR)

       # Beschneiden des Bildes auf die gleiche Größe wie das Originalbild
       x,y,w,h = roi
       dst = dst[y:y+h, x:x+w]
 ...
 #      for n in range(ITERATIONS):
  ...
  #         frame_small = crop_and_scale(frame, **CROP_AND_SCALE_PARAM)
   #        output = horizon_detector.find_horizon(frame_small, diagnostic_mode=False)
       ...
       # calc horizon
       frame_small = crop_and_scale(dst, **CROP_AND_SCALE_PARAM)
       output = horizon_detector.find_horizon(frame_small, diagnostic_mode=False)
       roll, pitch, variance, is_good_horizon, mask = output
       if roll!= None:
          roll=int(roll)
          pitch=int(pitch)
       #send horizon data to Rocket_ctrl
 #      mq_horizon.send(str(roll) + "/" + str(pitch) + "/" + str(is_good_horizon) + "/")


       #draw the horizon
       color = (255,0,0)
       draw_horizon(dst, roll, pitch, FOV, color, True)
       print(f'Calculated roll: {roll}')
       print(f'Calculated pitch: {pitch}')

       # draw center circle
       center = (dst.shape[1]//2, frame.shape[0]//2)
       radius = dst.shape[0]//100
